...
const facts = [
    ...
    },
    {
        ...
        content: "Некоторые женщины считают, что удар или ушиб груди может спровоцировать рак, однако научные исследования не подтверждают прямую связь. Травма может лишь помочь заметить уже существующую патологию, вызывая боль и привлекая внимание к проблемному месту.",
        icon: "❌",
    },
];
...
function FactCard({ fact, index }) {
    ...
    return (
        ...
        >
            <div className="bg-pink-100 rounded-lg shadow-lg p-6 flex flex-col items-center hover:scale-105 transition-all h-full">
                <div className="text-4xl mb-4">{fact.icon}</div>
                <h3 className="text-xl font-medium mb-4">{fact.title}</h3>
                <p className=" text-center">{fact.content}</p>
            </div>
        </motion.div>
    );
}
